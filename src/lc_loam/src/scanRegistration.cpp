      ...
      while(!pointCloudBuf.empty()){
        {
            //read data
            mutex_lock.lock();
            pcl::PointCloud<common::PointType>::Ptr pointcloud_in(
                                              new pcl::PointCloud<common::PointType>());
            // if(transform::PointCloud2HasField(*pointCloudBuf.front(), "intensity"))
            // {
                pcl::fromROSMsg(*pointCloudBuf.front(), *pointcloud_in);
            // }
            // else{
            //     pcl::PointCloud<pcl::PointXYZ>::Ptr pointcloud_in_copy(
            //                                   new pcl::PointCloud<pcl::PointXYZ>());
            //     pcl::fromROSMsg(*pointCloudBuf.front(), *pointcloud_in_copy);
            //     pointcloud_in->reserve(pointcloud_in_copy->points.size());
            //     for(int i = 0; i < (int) pointcloud_in_copy->points.size(); ++i){
            //          pointcloud_in->points[i].x = pointcloud_in_copy->points[i].x;
            //          pointcloud_in->points[i].y = pointcloud_in_copy->points[i].y;
            //          pointcloud_in->points[i].z = pointcloud_in_copy->points[i].z;
            //     }
            // }
...
void scanRegistration::featureExtraction(
                          const pcl::PointCloud<common::PointType>::Ptr& pc_in,
    ...
    std::vector<pcl::PointCloud<common::PointType>::Ptr> laserCloudScans;
    for(int i = 0;i< N_SCANS; i++){
        laserCloudScans.push_back(pcl::PointCloud<common::PointType>::Ptr(
                                            new pcl::PointCloud<common::PointType>()));
    }
    ...
    for (int i = cloudCurvature.size()- 1; i >= 0; i--)
    {
        ...
        if(std::find(picked_points.begin(), picked_points.end(), ind)==picked_points.end()){
            ...
            if (largestPickedNum <= 20){
                pcl::PointXYZI tmp;
                tmp.x = pc_in->points[ind].x;
                tmp.y = pc_in->points[ind].y;
                tmp.z = pc_in->points[ind].z;
                pc_out_edge->push_back(tmp);
                point_info_count++;
            }else{
                break;
            }
