...
PoseGraph::PoseGraph(std::unique_ptr<optimization::OptimizationProblem> optimization_problem,
                  common::ThreadPool *thread_pool, const common::PoseGraphOptions& options):
                  constraint_builder_(thread_pool, options.loop_closureOptions),
                  thread_pool_(thread_pool),
                  matcher_translation_weight(options.matcher_translation_weight),
                  matcher_rotation_weight(options.matcher_rotation_weight),
                  optimize_every_n_nodes(options.optimize_every_n_nodes),
                  optimization_problem_(std::move(optimization_problem))
                  {}
...
void PoseGraph::DrainWorkQueue() {
 ...
 void PoseGraph::HandleWorkQueue(const std::vector<Constraint>& result){
   ...
   // todo: 缺少一部分根据当前的优化结果调整未优化到的节点的位姿。
   DrainWorkQueue();
 }
 ...
 void PoseGraph::ComputeConstraint(const NodeId& node_id, const SubmapId& submap_id){
...

     {
       ...
       global_pose = data_.trajectory_nodes.at(node_id).global_pose;
       submap = static_cast<const Submap *>(
           data_.submap_data.at(submap_id).submap.get());
     }
          // TODO:
      constraint_builder_.MaybeAddConstraint(submap_id, submap, node_id, data_,
                                                 constant_data, global_pose, global_node_pose,
                                                 global_submap_pose, loop_closure_flag);
}
 ...
 WorkItem::Result PoseGraph::ComputeConstraintsForNode(
        ...
        std::set<NodeId> newly_finished_submap_node_ids; // 刚刚完成的子图对应的id
         // 节点与子图内的约束
        {
            absl::MutexLock locker(&mutex_);
            // 获取节点信息数据
  ...
  PoseGraph::SubmapData PoseGraph::GetSubmapDataUnderLock(const SubmapId submap_id) const{
      ...
      if(data_.global_submap_poses.Contains(submap_id)){
        return {submap, data_.global_submap_poses.at(submap_id).global_pose};
      }

      return {submap, ComputeLocalToGlobalTransform(data_.global_submap_poses,
  ...
  void PoseGraph::RunOptimization(){
     ...
     const auto local_to_new_global =
         ComputeLocalToGlobalTransform(submap_data, trajectory_id_);
     const auto local_to_old_global =
         ComputeLocalToGlobalTransform(data_.global_submap_poses, trajectory_id_);
     const transform::Rigid3d old_global_to_new_global =
         local_to_new_global * local_to_old_global.inverse();
     // 获取这次优化的最后一个node的id
     const NodeId last_optimization_node_id =
         std::prev(node_data.EndOfTrajectory(trajectory_id_))->id;
     auto node_it =
         std::next(data_.trajectory_nodes.find(last_optimization_node_id));
     // Step: 根据之前位姿的改变量，对没有优化过的位姿进行校正
     for (; node_it != data_.trajectory_nodes.EndOfTrajectory(trajectory_id_); ++node_it){
     ...
     // 更新所有submap的位姿
     data_.global_submap_poses = submap_data;
  }
  ...
  MapById<NodeId, TrajectoryNodePose> PoseGraph::GetTrajectoryNodePoses()
    const {
  MapById<NodeId, TrajectoryNodePose> node_poses;
  absl::MutexLock locker(&mutex_);
  for (const auto& node_id_data : data_.trajectory_nodes) {
    absl::optional<transform::Rigid3d> constant_pose_data;
    if (node_id_data.data.constant_data != nullptr) {
      constant_pose_data = transform::Rigid3d{
  ...
  void PoseGraph::lastOptimization(){
    //DrainWorkQueue();
