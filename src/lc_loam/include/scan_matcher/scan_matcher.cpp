...
namespace optimization{
    ...
    void CeresScanMatcher::Match(transform::Rigid3d* initial_pose_estimation,
        ...
        addEdgeCostFactor(pc_edge_in, submap->conerMap(), problem, loss_function, matcher_ok);
        addSurfCostFactor(pc_surf_in, submap->surfMap(), problem, loss_function, matcher_ok);
    ...
    void CeresScanMatcher::transformRangeData(pcl::PointXYZI const *const pi,
    ...
    for (int i = 0; i < (int)pc_in->points.size(); i++)
    {
        ...
        transformRangeData(&(pc_in->points[i]), &point_temp);
